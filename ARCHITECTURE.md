...
Шесть статов-процентов: `stat_damage`, `stat_health`, `stat_speed`,
`stat_damage_received`, `stat_healing_received`, `stat_projectiles_speed`.
База = 100. Лимиты — массив `stats_limit` (enum `StatLimit`: HEALTH/DAMAGE/SPEED).

Инициализация — в `core/init.opy`, в ДВУХ местах: `Player Init` (живые игроки) и
`Init Dummy Bot` (боты). Рост с уровнем — `core/leveling.opy`. Временные изменения —
через хребет (тип = соответствующий `EffectType`, action `SUBTRACT`, value = дельта).
...
| # | Шов | Статус |
|---|-----|--------|
| 1 | **Смерть не чистит `active_buff_*`.** `core/respawn.opy` при `playerDied` зовёт только `stopAllDamageOverTime()`. Если игрок умер с активным стат-баффом, после респауна бафф истечёт и `SUBTRACT` вычтет из стата → возможна порча стата. | ЗАКРЫТ (при смерти `*_expire_times` обнуляются, вызывая безопасный авто-откат) |
| 2 | **Нет правила `playerLeftMatch`.** При выходе игрока его HUD/эффекты не убираются принудительно → ghost-сущности в памяти сервера (нарушает `CODESTYLE §6.5`). | ОТКРЫТ |
| 3 | **`add_buff()` привязан к `eventPlayer`.** Дебаффы на чужую цель раньше требовали ручных `.append` (см. историю `cursed_mark`). | СМЯГЧЁН макросами `trackBuff`/`applyVisibleDebuff` (есть в `array_schemas.opy`); миграция `cursed_mark` на них — TODO |
