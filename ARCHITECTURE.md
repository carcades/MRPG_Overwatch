...
systems/
  effects_lifecycle.opy        ХРЕБЕТ: авто-удаление эффектов (API через макросы в array_schemas)
...
**Хранилище** — 8 параллельных массивов на игроке (индекс i = один эффект):
`active_buff_ids`, `active_buff_targets`, `active_buff_expire_times`,
`active_buff_types`, `active_buff_actions`, `active_buff_values`, `active_buff_sources`, `active_buff_flags`.
Схема индекса — enum `EffectEntry` в `array_schemas.opy`.

**Регистрация:**
- Вся регистрация проходит атомарно на любой цели через макросы `trackBuff(...)` и 
  `applyVisibleDebuff(...)` из `array_schemas.opy` (`victim.applyVisibleDebuff(...)`).
- **Per-Attacker эффекты:** Для эффектов, которые могут быть наложены несколькими игроками независимо (например, Cursed Mark), используются макросы `trackAttackerBuff(...)` и `applyVisibleAttackerDebuff(...)`. Они записывают `source` (создателя) в 7-й массив.
- **Перманентные эффекты:** Если эффект должен сохраняться после смерти цели (например, Пепельный покров), используйте стандартные макросы (например, `applyVisibleDebuff`), а **на следующей строке** вызовите модификатор `setLastVisibleFlag(BuffFlag.PERSIST_ON_DEATH)`. Он безопасно применит флаг к последнему добавленному баффу, благодаря однопоточной архитектуре Workshop.

**Продление:** Через макросы `extendBuff(buffId, newExpire)` (атомарно по первому индексу)
или `extendBuffAll(buffId, newExpire)` (обновление всех элементов). 
> **Внимание:** `extendBuff` уязвим к коллизиям `.index()`, поэтому для дублируемых эффектов используется **только** `extendBuffAll`. Валидатор защищает от ошибки через чтение маркера `# DUPLICATABLE` у `BuffId`.
- **Per-Attacker продление:** Используется `extendAttackerBuff(buffId, source, newExpire)` — безопасно продлевает эффект только для конкретного создателя.
...
6 параллельных массивов: `hud_buff_ids`, `hud_buff_expires`, `hud_buff_names`,
`hud_buff_colors`, `hud_buff_sources`, `hud_buff_flags`. Регистрация через макросы `trackHud(...)` / `applyVisibleDebuff(...)`
на любой цели. Для независимых эффектов от разных игроков — `trackAttackerHud(...)` / `applyVisibleAttackerDebuff(...)`.
Для применения флагов (например, `BuffFlag.PERSIST_ON_DEATH`) используйте модификатор `setLastHudFlag(...)` или `setLastVisibleFlag(...)` сразу после регистрации.
...
| 4 | **Гонки `temp_var` в suspendable-циклах.** Переиспользование `temp_var` в правилах с `wait()` вызывает краши при конкурентном выполнении. | ЗАКРЫТ (каждый цикл имеет выделенный слот: `hud_loop_iter`, `gc_loop_iter` и т.д.) |
| 5 | **`branch_data` — юнион двух раскладок.** Чтение «не той» ветки даёт тихий мусор. | ПРИНЯТО как дизайн; защита — дисциплина макросов |
| 6 | **`tier_up_reset` — заготовка** (`# stub`, без `def`). Сброс бонусов при переходе на тир выше пока не реализован. | ЗАПЛАНИРОВАН |
| 7 | **Инициализация продублирована** в `Player Init` и `Init Dummy Bot` (статы + `passives_data`). Новый стат/поле легко забыть в одном из блоков. Вдобавок бот-init **неполный**: нет `stats_limit`, `level_up_points`, `current_race` и др. — боты живут на zero-default. | ЗАКРЫТ (дублирование устранено: создана общая логика `init_common_player_fields()`, вызываемая обоими блоками) |
...
| F. HUD-связность | использование `trackBuff` для видимых эффектов без `applyVisibleDebuff` (эвристика по макросам) |
