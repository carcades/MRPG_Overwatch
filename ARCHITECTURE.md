...
| # | Шов | Статус |
|---|-----|--------|
| 1 | **Смерть не чистит `active_buff_*`.** `core/respawn.opy` при `playerDied` зовёт только `stopAllDamageOverTime()`. Если игрок умер с активным стат-баффом, после респауна бафф истечёт и `SUBTRACT` вычтет из стата → возможна порча стата. | ОТКРЫТ |
